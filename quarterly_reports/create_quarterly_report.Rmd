...
# 2021-12-05: Master log screened in imported with 329 rows and 41 columns.
```
...
# 2021-12-05: Master log screened out imported with 53 rows and 12 columns.
```
...
```{r}
# Don't worry about warnings converting character zeros to numeric zeros
ema <- read_excel(
  ema_15_day_path,
...
```{r}
rm(
  master_log_path, phone_term_path, remote_path, ema_initial_pay_approach_path,
  ema_15_day_path, arrest_path, bridge_path 
)
```
...
```{r}
master_log_screened_in  <- filter(master_log_combined, screened_in == 1L)
master_log_screened_out <- filter(master_log_combined, screened_in == 0L)
```
...
# Don't worry about
# Warning: Problem with `mutate()` column `date_session`.
# ℹ `date_session = as.numeric(date_session)`.
...
We will build out out the Word report in the analysis section below.

```{r}
doc <- read_docx("template_l2c_quarterly_report.docx")
```

Date the report was updated

```{r}
today <- format(Sys.Date(), "%B %d, %Y")
doc <- body_replace_text_at_bkm(doc, "date", today)
```
...
```{r}
key_dates <- master_log_combined %>% 
  summarise(
    date_first_screen = min(date_baseline) %>% as.character(),
    date_last_screen = max(date_baseline) %>% as.character()
  )
```
...
```{r}
screened <- master_log_combined %>% 
  summarise(
    n_screened = n(),
    screened_in = sum(screened_in),
    screened_out = n_screened - screened_in
  )
```
...
```{r}
randomized <- master_log_screened_in %>% 
  mutate(
    group_f = fct_explicit_na(group_f, na_level = "Not Randomized")
  ) %>% 
  freq_table(group_f) %>% 
